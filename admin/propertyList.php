    ...
    <div class="content">
        ...
        <table class="table">
            ...
            <tbody>
                ...
                $sql = "SELECT property.PROP_ID, property.ADVERTISER_ID, 
                            CONCAT(users.USER_FNAME, ' ', users.USER_LNAME) AS user_name, 
                            property.PROP_NAME, property.PROP_ADDRESS, property.status 
                        FROM property
                ...
                foreach ($properties as $row) {
                    echo "<tr>
                            <th scope='row'>" . htmlspecialchars($row['PROP_ID']) . "</th>
                            <td>" . htmlspecialchars($row['user_name']) . "</td>
                            <td>" . htmlspecialchars($row['PROP_NAME']) . "</td>
                            <td>" . htmlspecialchars($row['PROP_ADDRESS']) . "</td>
                            <td>" . htmlspecialchars($row['status']) . "</td>
                            <td>
                                <a class='button success-button' href='editPropertyList.php?PROP_ID=" . urlencode($row['PROP_ID']) . "'>Update status</a>
                                <a class='button view-button' href='approval/view.php?PROP_ID=" . urlencode($row['PROP_ID']) . "'>View</a>
                                <a class='button danger-button' href='deleteList.php?PROP_ID=" . urlencode($row['PROP_ID']) . "'>Delete</a>
                            </td>
                        </tr>";
                }
                ?>
